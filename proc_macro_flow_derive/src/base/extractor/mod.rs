...
// TODO[x](#extractor/entry):C[F(extractor).A(\1).T(DeriveInput)], "DONE. lib.rs::extractor parses a DeriveInput and calls StructExtraction::extract_from, and - since the last pass - actually READS the result rather than discarding it. Two corrections to the original wording: the type is Extracted<StructExtraction, &DeriveInput> and not ExtractionState, which no longer exists; and the traversal is no longer a Visit walk, because extract_from descends from the DeriveInput itself (ID(extractor/recursive-source))"
pub mod extractor;
pub mod generator;
pub mod processor;
